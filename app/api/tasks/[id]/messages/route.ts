 ...
 *
 * 用户中途/任务结束后追加消息。逻辑：
 *   1. 校验 session 存在且不在 running / queued 状态
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const userMsgs = prior
    .filter(e => e.type === 'chat' && e.sender === 'user' && e.content)
  ...
  let summary = '';
  if (prevUserMsgs.length > 0 || lastAssistantMsg) {
  ...
  void runAgent(params.id, fullPrompt).catch(err => {
